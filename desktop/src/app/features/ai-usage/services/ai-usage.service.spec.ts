...
function buildSummaryDto(overrides: Partial<AiUsageSummaryApiDto> = {}): AiUsageSummaryApiDto {
    return {
        ...
        byModel: [{ model: 'gemini-2.5-flash', requests: 4, totalTokens: 600, costUsd: 1.2 }],
...
describe('AiUsageService', () => {
    ...
    describe('getLogs', () => {
        it('should send page/size and only the filter params that are set', () => {
            ...
            service.getLogs(1, { model: 'gemini-2.5-flash', userId: undefined, startDate: '2026-01-01' })
            ...
            const req = httpMock.expectOne(
                (request) =>
                    ...
                    request.params.get('model') === 'gemini-2.5-flash' &&
    ...
    describe('getModels', () => {
        it('should return the list of distinct models from the response data', () => {
            ...
            req.flush(buildResponse(['gemini-2.5-flash', 'gemini-3.1-flash-lite']));

            expect(result).toEqual(['gemini-2.5-flash', 'gemini-3.1-flash-lite']);
        });
    });
});
